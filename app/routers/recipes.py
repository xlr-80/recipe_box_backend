...
async def create_review(
    ...
    # Fire-and-forget: notify the recipe's author, unless they reviewed
    # their own recipe. notifications.send_to_user is itself a no-op if
    # the author has no registered devices or FCM isn't configured, so
    # this never fails the request either way.
    if recipe.author_id != str(current_user.id):
        await notifications.send_to_user(
            recipe.author_id,
            title="New review on your recipe",
            body=f'{current_user.full_name} left a {payload.rating}-star review on "{recipe.title}"',
            data={"type": "new_review", "recipe_id": recipe_id},
        )
